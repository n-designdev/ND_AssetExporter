...
def get_tg_nodes(namespace, regexArgs):
    nodes = []
    all_objs = cmds.ls('*:*')
    for regex in regexArgs:
        print(namespace, regex)
        nodes.extend([i for i in all_objs[:] if re.search(r"{}:{}".format(namespace, regex), i)!= None])
...
def export_anim_main(**kwargs):
    ...
    hidden_cache = cmds.hide(cache_nodes, rh=True)
    ...
    for tg_ns in tg_ns_list:
        all_nodes += get_tg_nodes(tg_ns, regex_obj_list)
